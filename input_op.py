...
class Rescale(object):
  ...
  def __call__(self, sample):
    ...
    label = transform.resize(label, (new_h_1,new_w_1))
    no_label=transform.resize(no_label,(new_h_2,new_w_2))

    return {'input': img, 'associ': label, 'noassoci':no_label}

class ToTensor(object):
  """Convert ndarrays in samplcriterion = nn.BCELoss()e to Tensors."""
  ...
  def __call__(self, sample):
    ...
    return {'input': torch.from_numpy(image),
            'associ': torch.from_numpy(label),
            'noassoci':torch.from_numpy(no_label)}
